...
namespace EasyBilling.Application.Services
{
    public class InvoiceService(
        ...
        BlobStorageService blobStorageService) : IInvoiceService
    {
        ...
        public async Task<byte[]> GenerateInvoicePdfAsync(Guid invoiceId, CancellationToken cancellationToken = default)
{
    ...
    try
    {
        ...
        await _invoiceRepository.SaveInvoiceBlobAsync(new InvoiceBlob
        {
    ...
    catch (Exception ex)
    {
        throw new InvalidOperationException(
            $"PDF generated, but failed to upload/store. Details: {ex}",
            ex);
    }
    ...
    return pdfBytes;
        ...
        public async Task<byte[]> GetStoredInvoicePdfAsync(
            Guid invoiceId,
            CancellationToken cancellationToken = default)
        {
            var blobLoc = await _invoiceRepository.GetInvoiceBlobByIdAsync(invoiceId, cancellationToken);

            if (blobLoc is null)
                throw new InvalidOperationException("Invoice PDF is not stored yet. Generate it first.");

            return await _blobStorageService.DownloadAsync(
                blobLoc.ContainerName,
                blobLoc.BlobName,
                cancellationToken);
        }
